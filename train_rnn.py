...
class EncoderRNN(nn.Module):
    def __init__(self, emb_dim, h_dim, v_size, gpu=True, v_vec=None, batch_first=True):
        ...
        self.rnn = nn.GRU(emb_dim, h_dim, batch_first=batch_first,
                            bidirectional=True)
    ...
    def init_hidden(self, b_size):
        ...
        if type(self.rnn) is nn.LSTM:   # LSTM have cell state, but GRU don't have
    ...
    def forward(self, sentence, lengths=None):
        ...
        out, hidden = self.rnn(packed_emb, self.hidden)
    ...
    for idx, batch in enumerate(train_iter):
        ...
        if (idx + 1) % log_interval == 0:
            print('train epoch: {} [{}/{}], acc:{}, loss:{}'.format(
                ...
                correct.item() /float(log_interval * len(x)),
                loss.item()))
            correct = 0
...
def test_model(epoch, test_iter):
    ...
    print('test epoch:{}, acc:{}'.format(epoch, correct.item()/float(len(test))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='train.tokenized')
    ...
    train, test = data.TabularDataset(
            path=args.path, format='csv',  
            fields=[('qid', None),
                    ('text', TEXT),
                    ('label', LABEL)],
            skip_header=True).split()
    #train, test = datasets.IMDB.splits(TEXT, LABEL)    # use IMDB  
