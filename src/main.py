...
with open('./settings/path.json') as f:
    paths = json.load(f)
...
# Pre-processing
## Extra information
parser.add_argument("--add_POS",        type=bool_flag, default=False, help="")
parser.add_argument("--add_position",   type=bool_flag, default=False, help="")
parser.add_argument("--add_dependency", type=bool_flag, default=False, help="")
## Data augumentation
...
parser.add_argument("--concat",     type=str,       default="w|g|f|pg", help="Concat serveral embeddings")
...
# Model about Encoder
parser.add_argument("--encoder",    type=str, default='LSTM', help="The model for encoder")
## LSTM 
...
assert params.exp_path != ""
assert 0 <= params.input_dropout < 1
assert 0 <= params.lstm_input_dropout < 1
assert os.path.isfile(params.word2vec)
assert os.path.isfile(params.glove)
assert os.path.isfile(params.fasttext)
assert os.path.isfile(params.paragram)
...
# ------- bulid model -------
wordEmbedding, model = build_model(params, True)
# ------- trainer settings -------
trainer = Trainer(params, wordEmbedding, model)
trainer.train()
# ------- evaluator -------
evaluator = Evaluator(params, trainer)
